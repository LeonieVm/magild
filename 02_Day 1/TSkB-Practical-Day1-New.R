...
# Check assumptions for relevant variables: Only look at the actual variables, 
# not the ID variables Class and Student by removing the first 2 columns of 
# the dataset
...
c <- ggplot(Total, aes(x = Extraversion, y = Popular, col=factor(Gender))) +  geom_point()
...
# you'll need the ggthemes package
...
# The while modeling the dependence of the residual variance on extraversion
...
# Now let's deal with some outliers
...
# Try to fix analysis with robust se's, what do you notice?
covHC0_ol <- vcovHC(Regr1_ol)
coeftest(Regr1_ol, vcov = covHC0_ol)
...
#------------------------------------------------------------------------------#
## Exercise 1c: Questions from BMLR Chapter 7
#------------------------------------------------------------------------------#
...
# Below is the code for the examples given in the chapter. 

# Can you spot: 
# How we choose to select a quasibinomial instead of a binomial model, and what the effect is?
# How we  between dam effects influence results?
...
# Assignment 1:
# The data for the 25 different classes are stored in de the dataframes 
# Class1 - Class25. The dataframe Total contains the combined data of all 
...
# Assignment 2:
# Run regression analyses for each of the 25 classes separately using the lm 
...
# called "Coefficients". To acces them separately ask for 
# ResultsClass1$coefficients,
ResultsClass1$coefficients
# The intercept is the first value sotred under coefficients and can be asked 
# for by typing,
...
Coefficients <- matrix(NA, ncol = 3, nrow = 25) # First we create a matrix in 
# which we store the inetercepts and coefficients
colnames(Coefficients) <- c("Intercept", "Extraversion", "Gender") #  And then 
# we specify the columnnames.
# Then we automatically run lm on all the 25 classes and store the results
for (i in 1:25) {
  Coefficients[i,] <- lm(Popular ~ 1+Extraversion+Gender, 
                         data = eval(parse(text = paste("Class", i, sep = ""))))$coefficients
...
Mean <- matrix(NA, ncol = 2, nrow = 25) # First we create a matrix in which 
# we store the inetercepts and coefficients
colnames(Mean) <- c("Popular", "TeacherExperiece") #  And then we specify the 
# columnnames
# Then we automatically run lm on all the 25 classes and store the results
for (i in 1:25) {
...
lm(Mean[,1]~1+Mean[,2])
lm(Mean$Popular ~ 1+ Mean$TeacherExperience)

# The sample size is equal to the number of Classes, 25


# Assignment 5:
# Compare the intercept and regression coefficients of gender and extraversion 
# from a multilevel analysis to the average estimates across the 25 separate 
...
lmer(Popular ~ 1 + Gender + Extraversion + (1 | Class), data=Total)
...
lmer(Popular ~ 1 + teacherExp + (1 | Class), Total)
...
# differences while the regresison analysis on the mean scores was not.    



# Assignment 6:
# Run a multilevel regression analysis on the data of all classes 
# simulataneously (use the "Total" dataframe for this) using the lmer command. 
...
FixedEffects <- lmer(Popular ~ 1 + Gender + Extraversion + teacherExp + (1 | Class), Total)
...
# Check the normality of the errors in your final model. 
